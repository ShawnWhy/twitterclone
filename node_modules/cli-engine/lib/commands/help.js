...
Help topics, type ${this.out.color.cmd(this.config.bin + ' help TOPIC')} for more details:\n`);
  ...
  listNamespaceHelp(plugins) {
    ...
    for (var i = 0; i < plugins.length; i++) {
      ...
      if (plugin.topics) {
        this.out.log(renderList(plugin.topics.filter(t => !t.hidden).map(t => [plugin.cachedPlugin.namespace ? `${plugin.cachedPlugin.namespace}:${t.topic}` : t.topic, t.description ? this.out.color.gray(t.description) : null])));
      }
    }
  }
